            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'AWS_ACCESS_KEY_ID', variable: 'ACCESS_KEY'), string(credentialsId: 'AWS_SECRET_ACCESS_KEY', variable: 'SECRET_KEY')]){
                        ...
                        sh """ jq -r '.imageScanFindings.findings[] | select(.severity == "CRITICAL") | .name as $name | .findings[] | select(.severity == "CRITICAL") | "Vulnerability: ($name) - Description: (.description)"' all_findings_${BUILD_NUMBER}.json """
                    }
                }
            }
        }
